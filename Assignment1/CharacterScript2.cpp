#include "CharacterScript2.h"

CharacterScript2::CharacterScript2(ObjectState* state)
	: ObjectScript::ObjectScript(state)
{
}

void CharacterScript2::Update(double deltaTime)
{
	if (mState->mInputController->Released("2"))
	{
		mSelected = true;
	}
	else if (mState->mInputController->IsPressed("1") || mState->mInputController->IsPressed("3"))
	{
		mSelected = false;
	}
